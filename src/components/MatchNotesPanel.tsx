...
import { Plus, Trash2, Image, FileText, X } from 'lucide-react';
// Polyfill-free UUID approach
function genId() {
    ...
    <div className="space-y-5">
      ...
      <div>
        ...
        {notes.length > 0 && (
          <div className="space-y-2 mb-3">
            {notes.map((note) => (
              <div
                key={note.id}
                className="flex items-start gap-2 bg-[#161b22] rounded-lg p-3 group"
              >
                <p className="flex-1 text-sm text-[#e6edf3] leading-relaxed">{note.text}</p>
                <button
                  onClick={() => deleteNote(note.id)}
                  className="text-[#484f58] hover:text-[#f85149] opacity-0 group-hover:opacity-100 transition-all mt-0.5"
                >
                  <Trash2 size={12} />
                </button>
              </div>
        ...
        <div className="flex gap-2">
          ...
          <button
            ...
            className="self-end px-3 py-2 bg-[#21262d] hover:bg-[#30363d] disabled:opacity-40 text-[#e6edf3] text-sm rounded-lg border border-[#30363d] transition-colors"
          >
            <Plus size={14} />
          </button>
        </div>
        <p className="text-[10px] text-[#484f58] mt-1">⌘ + Enter to add</p>
      </div>
      ...
      <div>
            ...
            {images.map((img) => (
              <div key={img.id} className="relative group rounded-lg overflow-hidden border border-[#30363d]">
                ...
                <input
                  ...
                  placeholder="Caption…"
                  className="w-full bg-[#0d1117]/80 px-2 py-1 text-[11px] text-[#e6edf3] placeholder-[#484f58] focus:outline-none"
                />
              </div>
            ))}
          </div>
        ...
        <button
          ...
          className="flex items-center gap-2 px-3 py-2 border border-dashed border-[#30363d] hover:border-[#58a6ff] text-sm text-[#8b949e] hover:text-[#58a6ff] rounded-lg transition-colors w-full justify-center"
        >
          <Image size={14} />
          Upload photos
        </button>
